...
export async function ensureTables() {
  ...
  try {
    ...
    // 2. user_assets 表
    if (!(await tableExists(db, 'user_assets'))) {
      await db.execute(`
    ...
    // 3. auth_submissions 表
    if (!(await tableExists(db, 'auth_submissions'))) {
      await db.execute(`
    ...
    // 4. bank_cards 表
    if (!(await tableExists(db, 'bank_cards'))) {
      await db.execute(`
    ...
    // 5. recharge_records 表
    if (!(await tableExists(db, 'recharge_records'))) {
      await db.execute(`
    ...
    // 6. withdraw_records 表
    if (!(await tableExists(db, 'withdraw_records'))) {
      await db.execute(`
    ...
    // 7. stocks 表
    if (!(await tableExists(db, 'stocks'))) {
      await db.execute(`
    ...
    // 8. trade_orders 表（交易订单）
    if (!(await tableExists(db, 'trade_orders'))) {
      await db.execute(`
    ...
    // 9. admins 表
    if (!(await tableExists(db, 'admins'))) {
      await db.execute(`
